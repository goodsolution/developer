    ...
    private List<AdviceCategoryData> prepareAdviceCategoriesData(List<Map<String, Object>> rows) {
        ...
        for (Map<String, Object> row : rows) {
            list.add(new AdviceCategoryData(
                    ...
                    JdbcUtil.getString(row, "context"),
                    JdbcUtil.getString(row, "name"),
                    JdbcUtil.getString(row, "description"),
                    JdbcUtil.getString(row, "tags"),
                    JdbcUtil.getBigDecimal(row, "price"),
                    JdbcUtil.getString(row, "currency_code"),
                    JdbcUtil.getString(row, "period_code"),
                    JdbcUtil.getBigDecimal(row, "price_per_month"),
                    JdbcUtil.getBigDecimal(row, "price_per_quarter"),
                    JdbcUtil.getBigDecimal(row, "price_per_half_year"),
                    JdbcUtil.getBigDecimal(row, "price_per_year")));
        }
        return list;
    }
    private List<AdvicePurchasedCategoryData> prepareAdvicePurchasedCategoriesData(List<Map<String, Object>> rows) {
        ...
        for (Map<String, Object> row : rows) {
            list.add(new AdvicePurchasedCategoryData(
                    JdbcUtil.getLong(row, "id"),//Long id,
                    JdbcUtil.getString(row, "category_name"),// String name,
                    JdbcUtil.getString(row, "category_description"),// String description,
                    JdbcUtil.getLong(row, "category_id")// Long categoryId,
            ));
        }
        return list;
    }

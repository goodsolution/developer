...
function findCourses() {
    $.ajax({
        url: "/admin/api/crs/courses?&type=b" + preparePaginationUrl(),
        type: "get",
        dataType: "json",
...
function fillRow(course) {
    $('#records').append(
        "<tr>" +
            ...
            "<td class='align-middle'>" + prepareDuplicateButton(course.id) + "</td>" +
        "</tr>"
    );
}

function prepareDuplicateButton(id){
    return '<button type="button" class="btn btn-sm btn-secondary" onclick="duplicate(' + id + ')">Duplicate</button>';
}
...
function goToDetailsPage(id) {
    window.location.href = "/admin/buy-our-code/details/" + id;
}
...
function duplicate(id) {
        $.ajax({
            url: "/admin/api/crs/course/" + id + "/duplicate",
            method: "post",
            contentType: "application/json",
        })
            .done(function () {

$(document).ready(function() {
    trace('page.show', 'my-courses');
    findCourses();
});

function findCourses() {
    $.ajax({
        url: "/api/crs/courses-bought-by-logged-customer?&courseType=c",
        type: "get",
        dataType: "json",
...
function showInfoNoCourses() {
    ...
    '<div class="text-center">'+
        "<h6>" + lang.myCourses_noCourses + "</h6>"+
    '</div>'
    );
}
    ...
    $('#results').append(
        '<div class="col-lg-4 my-3">' +
            '<div class="text-center p-1 rounded bg-light shadow-sm product">' +
                ...
                '<div class="my-2">' +
                    '<div class="p-3">' +
                        '<button class="btn btn-outline-primary btn-lg btn-block" onclick="goToWatchPage(' + course.id + ')"><i class="fas fa-film"></i> ' + lang.myCourses_watch + '</button>' +
                    '</div>' +
                '</div>' +
            '</div>' +
        '</div>'

...
function appendCourse(course) {
    ...
    '<div class="col-lg-4 my-3">' +
        '<div class="text-center p-1 rounded bg-light shadow-sm product">' +
            '<div class="my-3" style="height:50px"><a class="product-title" href="/course-details/' + course.id + '">' + course.title + '</a></div>' +
            '<div style="width:200px;height:200px;" class="m-auto" onclick="goToDetailsPage(' + course.id + ')">' + prepareProductImage(course.imageName) + '</div>' +
...
function prepareCourseInteractionButton(course) {
    if(course.saleStatus === 'o') {
        ...
        return '<a type="button" class="btn btn-outline-success" href="/course-details/' + course.id + '"></i>' + lang.home_findOutMore + '</a>';
    }
}
